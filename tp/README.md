...
2. www.flower.com vhosts httpd-vhost  http.config
3. public/static  /static/css/index.css
4. ?m=index(module模块)&c=index(controller控制器)&a=index(action行为  方法)   /index/index/index
5. 在config.php中修改设置  将app_debug 改为  true
## 地址访问
1. 入口隐藏
2. 参数使用   路径  /name/zhangsan  
              查询字符串  ?name=zhangsan
3. url 生成    {:url('index/index/index')}
## 数据库的原生查询
1. 配置 database.php  数据库的名字
2. Db::  Db类的静态方法
3. query()  数组  查询结果
4. execute()   数值  操作影响的行数
## 数据库的构造器查询
1. Db::table()   Db::name()  设置要操作的表名
2. 查询   顺序：保证select  或者find 在最后    方法链式调用
    * field() 限制查询的列
    * where() 限制查询的行
        * where(id,1)
        * where(id,'=',1)  >=  <=  <>  like  in   between  
        * where (name,like,%$s%);  where(name,exp,like,%$s%)
        * where(id,in,[1,2,3])  where(id,exp,in(1,2,3))
        * where(id,between,[1,8])
        * where()->where()-where();  并且
        * where([id=>['=',1],age=>['>',20]]);id等于1或者age>20
        * where('id',[[in,[1,2,3]],[between,[5,8]]],'or') id是1,2,3或者在5-8中间
        * whereOr()->whereOr() 或者
    * order()  对于结果进行排序
    * limit()  对于结果进行行数的限制
    * select() 查询符合条件的数据的集合
    * find() 查询某条数据
3. 插入  insert([a=>1,b=>2])
4. 更新 where update([name=>""]);
5. 删除 where delete();
## 请求和响应
1. request()  获取当前所有的请求信息  助手函数   
2. input()  
    * input("get.")  获取所有信息
